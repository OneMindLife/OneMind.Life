-- =============================================================================
-- MIGRATION: Orchestrator trigger respects paused state
-- =============================================================================
-- Bug fix: When a chat is auto-paused (or schedule-paused), the orchestrator
-- trigger still fires on phase changes, causing agents to propose/rate in
-- paused chats. This wastes Gemini API credits.
--
-- Fix: Check is_chat_paused() before calling the orchestrator edge function.
-- The process-timers fix is in the edge function code (TypeScript), not SQL.
-- =============================================================================
...
BEGIN
  ...
  -- Skip if chat is paused (auto-pause or schedule-pause)
  SELECT is_chat_paused(v_chat_id) INTO v_paused;
  IF v_paused THEN
    RAISE LOG 'Agent orchestrator skipped for round % — chat % is paused', NEW.id, v_chat_id;
    RETURN NEW;
  END IF;
