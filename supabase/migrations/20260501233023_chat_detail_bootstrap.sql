-- =============================================================================
-- Chat detail bootstrap RPC.
-- =============================================================================
-- Replaces the 18 parallel queries that ChatDetailNotifier._loadData fires on
-- every chat-detail mount/refresh with one round-trip. The 18-query pattern
-- caused a cascade-and-stall problem at NCDD demo scale: each subscription
-- event re-triggered the full reload, the queries serialized on the connection
-- pool, and pause-resume could spend 13s draining backlog before the user saw
-- a phase change.
--
-- This RPC takes a snapshot in a single transaction and returns one JSONB
-- payload shaped exactly like the per-query results the Flutter notifier
-- already parses. The Flutter side keeps deserializing through existing
-- `fromJson` factories so model contracts stay unchanged.
--
-- SECURITY DEFINER with an explicit access check at the top: the caller must
-- be a participant of the chat OR the chat must be public. Otherwise NULL is
-- returned (mirrors today's RLS-filtered getChatById path).
-- =============================================================================
