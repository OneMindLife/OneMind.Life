...
VALUES ('Regular Chat 1', 'Not official', 'session-1');
...
VALUES ('Official OneMind', 'Humanity''s public square', 'official-session', TRUE);
...
SELECT extensions.throws_ok(
  $$INSERT INTO chats (name, initial_message, creator_session_token, is_official)
    VALUES ('Fake Official', 'Trying to be official', 'fake-session', TRUE)$$,
...
INSERT INTO chats (name, initial_message, creator_session_token, is_official)
VALUES ('Regular Chat 2', 'Also not official', 'session-2', FALSE);

SELECT extensions.is(
  (SELECT COUNT(*) FROM chats WHERE is_official = FALSE),
  2::bigint,
...
INSERT INTO participants (chat_id, session_token, display_name, is_host, is_authenticated, status)
VALUES
  (current_setting('test.official_chat_id')::INT, 'anon-1', 'User 1', FALSE, FALSE, 'active'),
  (current_setting('test.official_chat_id')::INT, 'anon-2', 'User 2', FALSE, FALSE, 'active');
