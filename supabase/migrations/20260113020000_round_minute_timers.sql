...
-- Ensures phase_ends_at always falls on :00 seconds to align with cron job schedule

-- Helper function to calculate round-minute end time
-- Given a duration in seconds, calculates the next :00 timestamp >= NOW() + duration
...
BEGIN
    v_min_end := NOW() + (duration_seconds * INTERVAL '1 second');
...
-- Update auto-start trigger to use round-minute calculation
...
BEGIN
    ...
    IF v_participant_count >= v_chat.auto_start_participant_count THEN
        ...
        -- Calculate phase end time (round up to next :00 for cron alignment)
        ...
        RAISE NOTICE '[AUTO-START] Created cycle % and round % for chat % with phase_ends_at %',
            v_new_cycle_id, v_new_round_id, NEW.chat_id, v_phase_ends_at;
    END IF;
...
'Automatically starts a chat (creates cycle/round) when participant count reaches auto_start_participant_count for auto mode chats. Uses round-minute timer alignment.';
