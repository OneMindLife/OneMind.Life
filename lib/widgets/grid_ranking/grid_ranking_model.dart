...
class GridRankingModel extends ChangeNotifier {
  ...
  RankingPhase _phase = RankingPhase.binary;
  List<RankingProposition> _rankedPropositions = [];
  ...
  factory GridRankingModel.fromResults(List<Map<String, dynamic>> propositions) {
    final model = GridRankingModel._forResults();
    ...
    for (final prop in propositions) {
      ...
      model._rankedPropositions.add(RankingProposition(
        id: id,
        content: content,
        position: rating,
        truePosition: rating,
    ...
    model._phase = RankingPhase.completed;
    model._detectStacks();
  ...
  RankingPhase get phase => _phase;
  List<RankingProposition> get rankedPropositions =>
  ...
  void addProposition(Map<String, dynamic> proposition) {
    ...
    if (_phase == RankingPhase.positioning) {
  ...
  void _initializeFromSavedRankings() {
    ...
    for (final prop in _inputPropositions) {
      ...
      _rankedPropositions.add(RankingProposition(
        id: id,
        content: content,
        position: position,
    ...
    if (_morePropositionsExpected) {
      _phase = RankingPhase.positioning;
  ...
  void swapBinaryPositions() {
    if (_phase != RankingPhase.binary || _rankedPropositions.length != 2) {
      return;
    }
  ...
  void confirmBinaryChoice() {
    if (_phase != RankingPhase.binary) return;
  ...
  void moveActiveProposition(double delta) {
    if (_phase != RankingPhase.positioning) return;
  ...
  void _applyPositionWithCompression(int activeIndex) {
    ...
    if (shouldCompress) {
      ...
      if (transitioningFromCompressed) {
        _expandCompressedPositions(activeIndex);
      } else {
  ...
  void _expandCompressedPositions(int activeIndex) {
    List<RankingProposition> inactiveProps = [];
    for (int i = 0; i < _rankedPropositions.length; i++) {
      if (i != activeIndex) {
  ...
  void undoLastPlacement() {
    ...
    if (_rankedPropositions.length == 2) {
      _phase = RankingPhase.binary;
      _currentPropositionIndex = 2;
    } else {
      var sortedByPlacementOrder = List<RankingProposition>.from(_rankedPropositions)
  ...
  void _uncompressAfterUndo() {
    ...
    var sortedByPosition = List<RankingProposition>.from(_rankedPropositions)
  ...
  void _spreadStackedCards(List<RankingProposition> props) {
  ...
  void _detectStacks() {
    ...
    final positionGroups = <double, List<RankingProposition>>{};
    ...
    for (var entry in positionGroups.entries) {
      if (entry.value.length > 1) {
        ...
        if (hasActiveProp) {
          ...
          final sortedByPlacement = List<RankingProposition>.from(entry.value)
            ..sort((a, b) => b.placementOrder.compareTo(a.placementOrder));
