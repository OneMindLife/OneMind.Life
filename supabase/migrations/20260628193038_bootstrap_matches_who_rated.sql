-- Fix: participants_who_rated must include matches-mode voters (rating_completions),
-- not just grid_rankings — else the rating-phase Done tag never shows in matches chats.
-- CREATE OR REPLACE based verbatim on 20260628143659 + a rating_completions union.
...
BEGIN
    ...
    IF v_current_cycle_id IS NOT NULL THEN
        ...
        IF v_current_round_id IS NOT NULL AND v_my_participant_id IS NOT NULL THEN
            ...
            -- Matches mode: pairwise voters finish via rating_completions
            -- (grid_rankings stays empty in matches mode), so union them in
            -- so the rating-phase Done tag works for matches chats too.
            SELECT COALESCE(
                (SELECT jsonb_agg(DISTINCT pid) FROM (
                    ...
                    WHERE rc.round_id = v_current_round_id
                      AND rc.participant_id IS NOT NULL
                ) u),
                '[]'::JSONB
            ) INTO v_participants_who_rated;
