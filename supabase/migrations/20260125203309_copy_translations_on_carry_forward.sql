-- Migration: Copy translations when carrying forward propositions
--
-- Problem: Carried-forward propositions don't have their own translations,
-- which breaks duplicate detection in the submit-proposition Edge Function.
-- The function checks for duplicates by comparing English translations in
-- the same round, but carried propositions have no translations.
--
-- Solution: When carrying forward a proposition, also copy its translations
-- from the original proposition (via carried_from_id).
...
COMMENT ON FUNCTION on_round_winner_set() IS 'Handles round completion: tracks consensus, creates next round, carries forward winners with their translations';
