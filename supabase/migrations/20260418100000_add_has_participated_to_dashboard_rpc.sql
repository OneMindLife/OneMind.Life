-- Migration: Add `has_participated` column to get_my_chats_dashboard.
--
-- Why: The home screen splits chats into "Your turn" (needs action) and
-- "Waiting on others". The Flutter side partitions based on
-- `ChatDashboardInfo.hasParticipated`, but that field was never populated
-- by the RPC, so it defaulted to `true` and every chat fell into "Waiting".
--
-- Semantics:
--   - No active round              -> TRUE  (nothing to do)
--   - Active round in 'waiting'    -> TRUE  (not yet actionable)
--   - Active round in 'proposing'  -> TRUE iff participant already submitted
--                                     a proposition for this round
--   - Active round in 'rating'     -> TRUE iff participant already submitted
--                                     ratings for this round (tracked via
--                                     round_participant_submissions)
