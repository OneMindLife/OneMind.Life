-- Migration: Add get_my_chats_dashboard RPC
-- Returns all chat fields + active round phase/timer + participant count
-- for the home screen realtime dashboard.
...
RETURNS TABLE (
    -- All chat columns from get_my_chats_translated
    id BIGINT,
    name TEXT,
...
BEGIN
    ...
    INNER JOIN participants p ON p.chat_id = c.id
        ...
        AND p.status = 'active'
    -- Translation joins (same pattern as get_my_chats_translated)
