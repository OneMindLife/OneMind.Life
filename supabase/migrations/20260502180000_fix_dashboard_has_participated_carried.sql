-- =============================================================================
-- Fix: get_my_chats_dashboard.has_participated treats carried-forward
-- propositions as "the user proposed."
-- =============================================================================
-- The previous proposing-phase check (20260418181311) was:
--
--   EXISTS (SELECT 1 FROM propositions
--           WHERE round_id = current AND participant_id = me)
--
-- Carried-forward propositions (NEW round, but proposition.participant_id =
-- the previous winner's author) match this. So the round-2 winner author
-- shows up on Home as has_participated = TRUE for round 3 proposing,
-- which puts the chat under "Coming up" even though they haven't acted
-- yet in round 3. They literally can't, until they propose or skip.
--
-- Mirror the same `carried_from_id IS NULL` filter we already use in
-- proposing_minimum counting and the participation-bar calculation.
-- One-line fix to the EXISTS clause; rest of the function is unchanged.
--
-- Confirmed via remote logs (perf_logs action='home.coming_up_entry')
-- on 2026-05-02 chat 364: the carry-prop owner was bucketed into Coming
-- up before they had touched round 3.
-- =============================================================================
    ...
    SELECT
        ...
        CASE
            ...
            WHEN ar.phase::TEXT = 'proposing' THEN (
                -- Submitted a NEW proposition (carried-forward winners
                -- don't count — the user hasn't actually acted in this
                -- round yet just because their previous round's winner
                -- got copied into this one).
                EXISTS (
